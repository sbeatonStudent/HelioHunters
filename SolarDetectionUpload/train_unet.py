...
def evaluate_model(model, dataset_path):
    ...
    predicted = model.predict(eval_gen)
    predicted_t = (predicted > 0.065).astype(np.uint8)
    count = 0
    for mask in predicted_t:
        png_path = "eval_dataset/eval/predicted_masks/"
        image = tf.keras.utils.array_to_img(mask)
        if mask.any() > 0:
            image.save(png_path + 'p_' + str(count) + '.png')
        count = count +1
...
def main():
    ...
    parser.add_argument("--model_name", type=str, default = "unet_seb_trans", help="Name of the model")
    parser.add_argument("--mode", type=str, default = "eval", help="Train/eval mode")
    ...
    if args.mode.lower() in ["t", "train"]:
        ...
        evaluate_model(model, "eval_dataset/eval/")
